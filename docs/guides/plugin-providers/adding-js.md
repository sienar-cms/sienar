...
Sienar enables developers to add JavaScript for their plugins with the [IScriptProvider](xref:Sienar.Infrastructure.Plugins.IScriptProvider) interface. This provider contains references to [ScriptResource](xref:Sienar.Infrastructure.Plugins.ScriptResource) objects, which are rendered in the order they were registered in the provider.
...
void Configure(WebApplication app)
{
	...
	app.ConfigureStyles(scriptProvider =>
    {
...
```html
<script
	src="https://www.googletagmanager.com/gtag/js?id=my-gtag-id"
	async></script>
```
