...
Sienar enables developers to add CSS for their plugins with the [IStyleProvider](xref:Sienar.Infrastructure.Plugins.IStyleProvider) interface. This provider contains references to [StyleResource](xref:Sienar.Infrastructure.Plugins.StyleResource) objects, which are rendered in the order they were registered in the provider.
...
{
	// Explicit StyleResource
	app.ConfigureStyles(styleProvider =>
    {
